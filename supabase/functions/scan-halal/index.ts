...
const HARAM_RULES = [
  ...
  {
    pattern: /\bcochineal\b/i,
    label: "Cochineal",
    note: "Cochineal/E120 is insect-derived and often treated as non-halal.",
  },
...
const MUSHBOOH_RULES = [
  ...
  {
    pattern: /\bmono[-\s]?glycerides?\b/i,
    label: "Mono-glycerides",
    note: "Mono-glyceride source may need verification.",
  },
...
function runDeterministicHalalCheck(productFacts: ProductLookup, body: ScanRequest): DeterministicResult {
  ...
  if (ingredients.length === 0) {
    ...
    return {
      ...
      confidence: 20,
      verdict:
        "OpenFoodFacts found this product, but no ingredient list was available for deterministic halal checking.",
      category: productFacts.category,
      region: productFacts.region ?? body.region ?? null,
      ingredients: [],
      ingredients_hash: null,
