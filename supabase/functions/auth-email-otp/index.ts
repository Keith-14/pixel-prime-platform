...
type Action = "request" | "verify" | "complete" | "signup";
...
Deno.serve(async (req) => {
  ...
  if ((action !== "request" && action !== "verify" && action !== "complete" && action !== "signup") || !email) {
  ...
  if (action === "signup") {
    ...
    const { error: roleError } = await supabase.from("user_roles").insert({ user_id: created.user.id, role });
  ...
  if (action === "request") {
    ...
    const { error: otpError } = await supabase.from("auth_email_otps").upsert(
      {
        email,
        code_hash: codeHash,
        attempts: 0,
  ...
  await supabase.from("auth_email_otps").delete().eq("email", email);
